...
export function getUserSession(req) {
  ...
  if (cookies.session) {
    ...
    if (global.session && global.session.username[0] === session.username[0] && new Date(global.session.loggedInAt).getTime() === new Date(session.loggedInAt).getTime()) {
      return session;
    }
  }
...
export function isConnected() {
  console.log('here' + global.session)
    if (global.session) {
        return true;
    }
    return false;
