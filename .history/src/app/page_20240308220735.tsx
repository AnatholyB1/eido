...
interface ComponentSelectorProps {
 ...
 function ComponentSelector({ TrueElement, FalseElement, validator }: ComponentSelectorProps) {
...
import {
 ...
 function DialogStats({ data, createNew }: PropsDialogStats) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    if (data)
    {
      updateUserStatById(data._id, values).then(() => {
  ...
  return (           
    <DialogContent className='px-2'>
      <DialogHeader>
        <DialogTitle>{data ? data.nom : 'Nouveau'}</DialogTitle>
              <Form  {...form} >
                    ...
                    <FormItem>
                      ...
                      {item.type == 'selector' ? 
                          <Select onValueChange={field.onChange} defaultValue={field.value as string}  >
                              <FormControl>                  
                                <SelectTrigger className='flex items-start w-full'>
                                  <SelectValue  placeholder='select a boss name'/>
                                </SelectTrigger>
                              </FormControl>  
                              <SelectContent>
                                  <SelectGroup>
                        ...
                        <FormControl> 
                          <Input
                            className={`flex items-start ${item.type == 'checkbox' ? 'w-auto' : 'w-full'}`}
                            type={item.type} disabled={item.disabled}
                            {...field}
                            checked={item.type == 'checkbox' ? field.value as boolean : false}
                            value={item.type == 'checkbox' ? item.name : item.type == 'number' ? field.value as number : field.value as string}
                            onChange={(event) => {  
                              const target = item.type == 'checkbox' ? !field.value : item.type == 'number' ? Number(event.target.value) : event.target.value 
...
import { updateUserStatById } from '@/data/updateStats';
import { createUserStat } from '@/data/createStats';
import { getBossNames } from '@/data/getBossNames';
