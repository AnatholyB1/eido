...
const navbarLinks: NavbarLink[] = [
    ...
    { href: '/dashboard', label: 'Dashboard', isNotAdmin: true },
    { href: '/admin', label: 'Admin', isNotAdmin: true },
];
...
export function NavbarLink({ children, className, href, ...props }: NavbarLinkProps) {
    return (
        <Link
            className={cn(className, `px-4 text-lg font-medium ${props.isNotAdmin &&'hidden'}`)}
            href={href}
            {...props}  
        >
            {children}
...
export function Links({ children, className, links, ...props }: LinksProps) {
    return (
        <div className={cn(className, 'flex flex-row items-center justify-around' )} {...props}>
