...
export function BurgerMenu({ className, children, ...props }: NavbarProps) {
    return (
             ...
             <Sheet {...props}>
                <SheetTrigger className={cn('flex flex-row items-center justify-around', className)}  >Open</SheetTrigger>
                <SheetContent>
                    <SheetHeader>
...
export default function Navbar({  className,children, ...props }: NavbarProps) {
    return (
        <div className={cn( 'flex flex-row items-center sm:justify-between justify-end w-full h-[5rem] border-b shadow-sm fixed bg-white/95 py-2 px-6',className )} {...props}>
